...
firstBallEmitted = true;
  let emitRate = forceEmit ? true : Math.random() < 0.006;

  if (player == 1 && emitRate) {
    createBall(-50, 100, 3);
  }
}
...
function draw() {
  if (document.getElementById("win").style.opacity > 0) {
    document.getElementById("win").style.opacity -= 0.01;
  }
  if (document.getElementById("loss").style.opacity > 0) {
    document.getElementById("loss").style.opacity -= 0.01;
  }
...
function drawPlayer() {
  ...
  bezierVertex(
    ...
    spring1.pos - mouseX,
    -100 + upSpring.transform(jumper.pos.y)
  );
...
function createBall(x, y, speed, radius = 50) {
  let b = new Ball(x, y, speed, radius);
  BALLS.push(b);
  return b;
}
